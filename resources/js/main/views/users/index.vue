<template>
	<AdminPageHeader>
		<template #header>
			<a-page-header :title="$t(`menu.${langKey}s`)" class="p-0">
				...
				" #extra>
					<a-space>
						<!-- <ImportUsers :pageTitle="importPageTitle" :sampleFileUrl="sampleFileUrl" :importUrl="importUrl"
	...
	<a-card class="page-content-container">
		<AddEdit :addEditType="addEditType" :visible="addEditVisible" :url="addEditUrl" @addEditSuccess="addEditSuccess"
			@closed="onCloseAddEdit" :addEditData="formData" :data="viewData" :pageTitle="pageTitle"
		...
		<a-row v-if="userType && userType == 'users'">
			<a-col :span="24">
				<a-tabs v-model:activeKey="searchStatus" @change="setUrlData">
					...
					<a-tab-pane v-for="usrStatus in userStatus" :key="usrStatus.key"
		...
		<a-row :gutter="[15, 15]" style="mb-20">
			...
			<a-col v-if="userType && userType != 'users'" :xs="24" :sm="24" :md="12" :lg="4" :xl="4">
				<a-select v-model:value="searchStatus" @change="setUrlData" show-search style="width: 100%"
					:placeholder="$t('common.select_default_text', [$t('user.status')])" :allowClear="true"
					optionFilterProp="label">
					<a-select-option v-for="usrStatus in userStatus" :key="usrStatus.key" :value="usrStatus.key"
		...
		<a-row :gutter="[15, 15]" class="mt-20">
			<a-col :span="24">
				<div class="table-responsive">
					<a-table :columns="tableColumns" :row-key="(record) => record.xid" :data-source="table.data"
