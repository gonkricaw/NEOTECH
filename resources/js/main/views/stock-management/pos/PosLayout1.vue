<template>
	<a-row :gutter="[8, 8]">
		<a-col :xs="24" :sm="24" :md="24" :lg="24" :xl="24" class="pos-style-1-category-scroll">
			<div @click="
					() => {
				...
				<p class="pos-catagory-namexx">{{ $t("common.all") }}</p>
			</div>

			<div v-for="category in categories" :key="category.xid" @click="
					() => {
				...
				<a-tooltip :title="category.name">
					<a-avatar size="large" :src="category.image_url" :style="{
							verticalAlign: 'middle', marginTop: '5px',
						}" />
					<p class="pos-catagory-namexx">{{ getCategoryName(category.name) }}</p>
				</a-tooltip>
			</div>
		</a-col>
	</a-row>
