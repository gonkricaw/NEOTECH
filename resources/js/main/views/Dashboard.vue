<template>
	...
	<div class="dashboard-page-content-container">
		...
		<a-row :gutter="[8, 8]" class="mt-30 mb-10">
			<a-col :xs="24" :sm="24" :md="12" :lg="6" :xl="2">
				Date Filter
			</a-col>
			<a-col :xs="24" :sm="24" :md="12" :lg="6" :xl="6">
				<DateRangePicker ref="serachDateRangePicker" @dateTimeChanged="
				...
				<a-col :xs="24" :sm="24" :md="12" :lg="6" :xl="6">
					<StateWidget>
						...
						<template #description>
							<div class="fonttypea" v-if="responseData.stateData">
								{{
			...
			<a-col :xs="24" :sm="24" :md="12" :lg="18" :xl="18">
				<a-card :title="$t('dashboard.sales_purchases')">
					...
					<template v-if="
						...
						" #extra>
						<a-button class="mt-10" type="link" @click="
								$router.push({
		...
		<a-row class="mt-30 mb-20" v-if="
			...
			">
			<a-col :span="24">
				<a-card :title="$t('dashboard.recent_stock_history')" :bodyStyle="{ paddingTop: '0px' }">
					<template #extra>
						<a-tabs v-model:activeKey="activeOrderType">
							<a-tab-pane v-if="
									permsArray.includes('sales_view') ||
									permsArray.includes('admin')
								" key="sales" :tab="$t('menu.sales')" />
							<a-tab-pane v-if="
									permsArray.includes('purchases_view') ||
									permsArray.includes('admin')
								" key="purchases" :tab="$t('menu.purchases')" />
							<a-tab-pane v-if="
									permsArray.includes('purchase_returns_view') ||
									permsArray.includes('admin')
								" key="purchase-returns" :tab="$t('menu.purchase_returns')" />
							<a-tab-pane v-if="
									permsArray.includes('sales_returns_view') ||
									permsArray.includes('admin')
								" key="sales-returns" :tab="$t('menu.sales_returns')" />
						</a-tabs>
					</template>
					<a-row>
						<a-col :xs="24" :sm="24" :md="12" :lg="6" :xl="6" class="col-border-right pt-30">
							<a-row v-if="responseData.stockHistoryStatsData" style="margin-right: 24px"
								class="stock-history-stats">
								<a-col :span="24" class="sales mb-20">
									<a-statistic :title="$t('dashboard.total_sales_items')" :value="
											responseData.stockHistoryStatsData.totalSales
										" />
								</a-col>
								<a-col :span="24" class="sales-returns mb-20">
									<a-statistic :title="$t('dashboard.total_sales_returns_items')" :value="
											responseData.stockHistoryStatsData
												.totalSalesReturn
										" />
								</a-col>
								<a-col :span="24" class="purchases mb-20">
									<a-statistic :title="$t('dashboard.total_purchases_items')" :value="
											responseData.stockHistoryStatsData
												.totalPurchases
										" />
								</a-col>
						...
						<a-col :xs="24" :sm="24" :md="12" :lg="18" :xl="18">
							<OrderTable :orderType="activeOrderType" :filters="filters" :perPageItems="5" />
						</a-col>
					</a-row>
				</a-card>
			</a-col>
		...
		<a-row :gutter="[18, 18]" class="mt-30 mb-20">
			<a-col :xs="24" :sm="24" :md="12" :lg="16" :xl="16">
				<a-card :title="$t('menu.stock_alert')" :bodyStyle="{ padding: '0px' }"
					v-if="responseData && responseData.stockAlerts">
					<a-table :columns="stockQuantityColumns" :row-key="(record) => record.xid"
						:data-source="responseData.stockAlerts" :pagination="false">
					...
					<template v-if="
							...
							permsArray.includes('admin')
						" #extra>
						<a-button class="mt-10" type="link"
			...
			<a-col :xs="24" :sm="24" :md="12" :lg="8" :xl="8">
				<a-card :title="$t('dashboard.top_customers')" :bodyStyle="{ padding: '0px' }"
					v-if="responseData && responseData.topCustomers">
					<a-table :columns="topCustomerColumns" :row-key="(record) => record.customer_id"
						:data-source="responseData.topCustomers" :pagination="false">
						<template #bodyCell="{ column, record }">
